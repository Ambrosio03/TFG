              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">
                    {pedido.fecha_creacion ? new Date(pedido.fecha_creacion).toLocaleDateString() + ' a las ' + new Date(pedido.fecha_creacion).toLocaleTimeString() : 'Desconocida'}
                  </p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Fecha de envío</p>
                  <p className="mt-1 text-sm text-gray-900">
                    {pedido.fecha_envio ? new Date(pedido.fecha_envio).toLocaleDateString() + ' a las ' + new Date(pedido.fecha_envio).toLocaleTimeString() : 'No ha sido enviado'}
                  </p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Fecha de entrega</p>
                  <p className="mt-1 text-sm text-gray-900">
                    {pedido.fecha_entrega ? new Date(pedido.fecha_entrega).toLocaleDateString() + ' a las ' + new Date(pedido.fecha_entrega).toLocaleTimeString() : 'No ha sido entregado'}
                  </p>
                </div>
                <div>
                ...
                {pedido.direccion ? (
                  <>
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-900">{pedido.direccion.nombre}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Dirección</p>
                      <p className="mt-1 text-sm text-gray-900">{pedido.direccion.direccion}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Ciudad</p>
                      <p className="mt-1 text-sm text-gray-900">{pedido.direccion.ciudad}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Código Postal</p>
                      <p className="mt-1 text-sm text-gray-900">{pedido.direccion.codigoPostal}</p>
                    </div>
                  </>
                ) : (
                  <p className="text-sm text-gray-500">No hay dirección de envío disponible.</p>
                )}
              </div>
            </div>
          </div>
                ...
                <div key={item.id} className="p-6">
                  <div className="flex items-center">
                    <div className="flex-shrink-0 h-20 w-20">
                      <img
                        src={item.imagen ? `http://localhost:8000/images/products/${item.imagen}` : '/placeholder.png'}
                    ...
                    <div className="ml-6 flex-1">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-gray-500">Cantidad: {item.cantidad || 0}</p>
                        </div>
...
export default MisPedidoDetalle; 